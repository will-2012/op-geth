...
const (
	// mergeBlockInterval defines the interval of block in one multiDifflayer.
	mergeBlockInterval = 1800

	// mergeWithdrawalProofBlockInterval defines the interval of withdrawal proof block.
	mergeWithdrawalProofBlockInterval = 3600

	// mergeMultiDifflayerInterval defines the interval to collect nodes to flush disk.
	mergeMultiDifflayerInterval = 3

	// reserveMultiDifflayerNumber defines the reserve number of multiDifflayer in node list.
	reserveMultiDifflayerNumber = 3
)
...
func (nf *nodebufferlist) commit(root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node) trienodebuffer {
	...
	nodeBufferLayerGauge.Update(int64(nf.layers))
	nodeBufferLastStateIdGauge.Update(int64(nf.stateId))
	nodeBufferLastBlockGauge.Update(int64(nf.block))
...
func (nf *nodebufferlist) revert(db ethdb.KeyValueReader, nodes map[common.Hash]map[string]*trienode.Node) error {
	...
	nodeBufferListSizeGauge.Update(int64(nf.size))
	nodeBufferCountGauge.Update(int64(nf.count))
	nodeBufferLayerGauge.Update(int64(nf.layers))
...
func (nf *nodebufferlist) flush(db ethdb.KeyValueStore, clean *fastcache.Cache, id uint64, force bool) error {
	...
	nodeBufferLayerGauge.Update(int64(nf.layers))
	if nf.layers > 0 {
		nodeBufferLastDifflayerAvgSize.Update(int64(nf.size / nf.layers))
	}
	baseNodeBufferSizeGauge.Update(int64(nf.base.size))
	baseNodeBufferLayerGauge.Update(int64(nf.base.layers))
...
func (nf *nodebufferlist) setSize(size int, db ethdb.KeyValueStore, clean *fastcache.Cache, id uint64) error {
	return errors.New("nodebufferlist not supported")
}
...
func (nf *nodebufferlist) diffToBase() {
	commitFunc := func(buffer *multiDifflayer) bool {
		...
		if nf.count < reserveMultiDifflayerNumber {
			log.Debug("node buffer list less, waiting more difflayer committed")
			return false
		}
...
// disableFlushing stops background write disk.
func (nf *nodebufferlist) disableFlushing() {
	nf.stopFlushing.Store(true)
	for nf.isFlushing.Load() {
		time.Sleep(time.Second)
		log.Warn("waiting background node buffer flushed into disk")
...
// enableFlushing enable background write disk.
func (nf *nodebufferlist) enableFlushing() {
	nf.stopFlushing.Store(false)
}

// withdrawalProofReader return the world state Reader of withdrawal block.
func (nf *nodebufferlist) withdrawalProofReader(blockRoot common.Hash) (layer, error) {
	...
	if nf.count < mergeMultiDifflayerInterval {
		withdrawalProofReaderLessDifflayer.Mark(1)
		log.Warn("failed to get withdrawal proof reader", "node buffer list count", nf.count)
		return nil, errNoWithdrawalProofDifflayer
	}
	...
	find := func(buffer *multiDifflayer) bool {
		...
		if buffer.block%mergeWithdrawalProofBlockInterval == 0 {
	...
	nf.traverse(find)
	if diff == nil {
		withdrawalProofReaderMismatch.Mark(1)
		log.Error("withdrawal proof state is not available", context)
		return nil, fmt.Errorf("withdrawal proof state %#x is not available", blockRoot)
	}
	withdrawalProofReaderSuccess.Mark(1)
	return &withdrawalProofReader{
		nf:   nf,
		diff: diff,
	}, nil
}
...
var _ layer = &withdrawalProofReader{}

type withdrawalProofReader struct {
	nf   *nodebufferlist
	diff *multiDifflayer
}

func (w *withdrawalProofReader) Node(owner common.Hash, path []byte, hash common.Hash) ([]byte, error) {
	w.nf.mux.RLock()
...
func (w *withdrawalProofReader) rootHash() common.Hash { return w.diff.root }
func (w *withdrawalProofReader) stateID() uint64 {
	return w.diff.id
}
func (w *withdrawalProofReader) parentLayer() layer { return nil }
func (w *withdrawalProofReader) update(root common.Hash, id uint64, block uint64, nodes map[common.Hash]map[string]*trienode.Node, states *triestate.Set) *diffLayer {
	return nil
}
func (w *withdrawalProofReader) journal(io.Writer) error { return nil }
...
func (mf *multiDifflayer) commit(root common.Hash, id uint64, block uint64, layers uint64, nodes map[common.Hash]map[string]*trienode.Node) error {
	if mf.id != 0 && mf.id >= id {
		log.Crit("state id out of order", "pre_stateId", mf.id, "capping_stateId", id)
	}
	if mf.block != 0 && mf.block >= block {
		log.Crit("block number out of order", "pre_block", mf.block, "capping_block", block)
	}

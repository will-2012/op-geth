...
type trienodebuffer interface {
	...
	withdrawalProofReader(blockRoot common.Hash) (layer, error)
}

func NewTrieNodeBuffer(db ethdb.Database, sync bool, limit int, nodes map[common.Hash]map[string]*trienode.Node, layers uint64) trienodebuffer {
	...
	log.Info("new node buffer list", "limit", common.StorageSize(limit), "layers", layers)
	return newNodeBufferList(db, uint64(limit), nodes, layers)
}
